...
let make_tiny s =
  ...
  with
    Module [LetDef (n, _) as h]
  | Module [LetRecDef (n, _) as h]
...
let pervasives =
  [make_tiny "let ref = fun __PER__x -> {contents = __PER__x}";
   ...
   make_tiny "let failwith __PER__s = raise (Failure __PER__s)";
   make_tiny "let invalid_arg __PER__s = raise (Invalid_argument __PER__s)"]
